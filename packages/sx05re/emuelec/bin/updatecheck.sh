...
UPDINFO="https://raw.githubusercontent.com/british-choi/emuelec.github.io/master/settings/EE_update"
UPDURL="https://github.com/british-choi/EmuELEC/releases/download/v"
TEST_UPDURL="https://github.com/british-choi/EmuELEC-tests/releases/download/v"
...
if [[ "$arguments" == *"forceupdate"* ]]; then
if [ $(get_ee_setting system.language) == "ko_KR" ]; then
text_viewer -e -w -t "에러!" -f 24 -m "업데이트 파일이 없거나 인터넷에 연결되어 있지 않습니다! 강제 업데이트가 중단되었습니다!\n\n에뮬레이션스태이션이 재시작됩니다!"
else
...
if [[ "$arguments" == *"forceupdate"* ]]; then
    if [ $(get_ee_setting system.language) == "ko_KR" ]; then
    text_viewer -y -w -t "강제 업데이트!" -f 24 -m "강제 업데이트를 실행했습니다. 업데이트 타입(STABLE 또는 BETA)에 따라 최신 릴리스가 다운로드됩니다.\n\n업데이트 파일이 성공적으로 다운로드되면 시스템이 재부팅됩니다.\n\n계속하시겠습니까?"
    else
